...
5. [MEASURED] 完成巴威 16 景 CyclObs SAR 的 `V·R` 时序；12 景通过眼区与中心质量门槛，4 景满足探索性双峰阈值。

## 最先报告的否定结果

- [MEASURED] **持续性概率基线被主检验否定。** 气候基线 Brier 为 `0.001311`，持续性为 `0.001313`；配对差值为 `+0.00000204`，风暴聚类 95% CI `[+0.00000111,+0.00000309]`。Brier 越低越好。
- [MEASURED] **巴威当前 SAR 序列支持 1 个观测双环时段。** 四个双峰过境集中在 7 月 8 日 09:19 UTC 至 7 月 9 日 21:27 UTC。原任务中的“3-4 次置换”数量主张由当前结构证据否定。
...
四个达标景之间缺少一景“质量合格且明确单峰”的分隔证据，因此它们构成一个观测双环时段。外峰半径呈 `141→106→104→123 km`，完整收缩与替代序列尚未由这些离散过境闭合。7 月 11 日景提供后期宽风圈结构证据，其 prominence 维持在预注册阈值下。
...
3. **拿什么证伪？** 风暴分组折外 Brier 已证伪持续性增益；公开产品字段审计约束标签可得性；质量合格 SAR 序列约束巴威双环时段数。
...
- 巴威 SAR 采样间隔无法闭合完整 ERC 生命周期；TC PRIMED 85-92 GHz ring-score 时序是下一条零人工标签结构通道。
