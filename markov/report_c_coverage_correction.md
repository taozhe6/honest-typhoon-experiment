...
2. [MEASURED] CyclObs 的一手裁决为：**确证 1 个连续双环风结构观测时段；2 个二手待核窗口不可判定；0 个待核窗口缺少时间命中。**
3. [MEASURED] TC PRIMED 在 2026-07-15 12:50 UTC 的 `v01r01/preliminary/2026/WP/09/` 文件数为 0，因此它对三个待核窗口均无时间或空间覆盖。
4. [MEASURED] 16 景 CyclObs SAR 的真实跨度为 2026-07-02 07:25 至 2026-07-11 09:52 UTC；12 景质量合格，4 景确认双环风结构。
5. [MEASURED] 二手叙述与一手结果已分栏保存；NASA SVS、Wikipedia 和 CIMSS 均从一手 tally 中排除。
...
|裁决|数量|范围|
|---|---:|---|
|确证的连续双环风结构观测时段|**1**|2026-07-08 09:19 至 2026-07-09 21:27 UTC|
|不可判定的二手待核窗口|**2**|7 月 4 日、7 月 7 日|
|缺少 CyclObs 时间命中的待核窗口|**0**|三个窗口均至少有一景时间命中|

[MEASURED] “1 个观测时段”来自既有规则：质量合格双峰景之间只有质量不合格景，没有质量合格单峰景负责关闭区间。该数字描述采样记录，不能转换成物理 ERC 周期数。
...
TC PRIMED 在本次快照中的必报格式为：确证 0 个；存疑 0 个；无覆盖 3 个待核窗口。
...
这些来源用于提出待核窗口。它们不进入“确证 1、存疑 2、无覆盖 0”的一手 tally。
...
- 当前一手资料只支持至少一个双环风结构观测时段。巴威完整 ERC 周期数保持不可判定。
