...
|C-代理|5--15% 强度波形标签 v2；密封验证集非退化 Brier 基线|`research-baseline / unvalidated`|输出未来 24 h 减弱-再增强波形概率，并接受密封 Brier 评分|
|C-结构|巴威 16 景 SAR `V·R` 时序；一手覆盖纠错；逐时段三类裁决|`research-measured`|只对可观测时段报告确证、存疑或无覆盖|
...
[MEASURED] C-代理标签 v2 结果发布于 `5b89e33`。
[MEASURED] C-结构覆盖纠错发布于 `78669fd`。
[MEASURED] A v3 发布于 `e405ba0`；`theta` 传播发布于 `329b77e`；
三重天花板发布于 `e3b610e`。
...
发布物：[C-代理 v1 失败基线](markov/report_c_branch.md)、
[C-代理标签 v2 报告](markov/report_c_event_label_v2.md)与
[C-代理 v2 可靠性图](markov/outputs/c_event_label_v2/validation_reliability.png)。
...
- [MEASURED] C-代理的输出是未来减弱-再增强强度波形概率。
- [MEASURED] 该标签不是 ERC。ERC 是可能成因之一，环境变化与中心分析跳变也能产生同形波动。

### 缺口与下一步

- 阈值量化严重，开发/验证发生率发生明显时变；当前资格保持 `unvalidated`。
- C-代理后续方法需要在密封时间样本上击败气候 Brier，并报告可靠性。

## C-结构：一手双环结构观测

发布物：[C-结构一手覆盖纠错](markov/report_c_coverage_correction.md)与
[C-结构巴威覆盖图](markov/outputs/c_coverage_correction/bavi_primary_coverage_timeline.png)。

### 已经能用的东西

- [MEASURED] 巴威共有 16 景 CyclObs SAR；12 景质量合格，4 景满足双峰阈值。
- [MEASURED] 7 月 8--9 日可观测时段裁决为“确证双环风结构”。
- [MEASURED] 7 月 4 日可观测时段裁决为“存疑”：眼区没有进入 SAR 获取范围。
- [MEASURED] 7 月 7 日可观测时段裁决为“存疑”：中心质量和眼区门槛没有同时通过。
- [MEASURED] TC PRIMED WP09/2026 preliminary 在审计时裁决为“无覆盖”：文件数为 0。
- [CITED] NASA/Wikipedia/CIMSS 叙述只作为待核线索，未进入一手结构裁决。

### 缺口与下一步

- 巴威 SAR 的离散采样尚未闭合完整结构生命周期。
- C-结构继续按可观测时段输出“确证/存疑/无覆盖”，不换算物理 ERC 次数。
- TC PRIMED 原始 85--92 GHz 轨道公开后，按眼区覆盖与双峰门槛逐景复核。
...
|C-代理|9 点 1 分钟 best-track 强度窗口|标签拟合参数 0；开发期选 1 个 `(H,q)`；验证 70 个台风|密封 Brier、可靠性、量化审计和基准率漂移|
|C-结构|SAR `(V,R,peaks,prominence,eye_coverage)`|拟合参数 0；16 景离散一手观测|逐景质量门槛与确证/存疑/无覆盖裁决|
|Markov v0.1|`(V,m,Pc,RMW,Z)`|2 个未拟合 demo 参数；只扫描 1 个可识别 `theta=Ck/h`|`dV/dt` 对 `Z` 的零导数、61 点终值传播、未来密封回报|

## 预注册与偏离

- [MEASURED] A 的规则、资格 manifest、误差计算按 `054f225 -> d30ff7e -> f1449fa` 分阶段提交。
- [MEASURED] A v3 的 DYC2 审计与 UKM 设计先于结果冻结于 `b11ab75`，结果发布于 `e405ba0`。
- [MEASURED] B 的协议先于结果提交于 `7d9a2ec`；公开真值覆盖为零后按协议终止 Tier 2 匹配。
- [MEASURED] C 的协议与参数预算修订先于结果提交于 `28d8b5b` 和 `4499af7`；四项网页/下载偏离均在分支报告逐项登记。
- [MEASURED] C-代理标签 v2 设计先于结果冻结于 `0ddeb14`，结果发布于 `5b89e33`。
- [MEASURED] C-结构覆盖纠错发布于 `78669fd`。
- [MEASURED] 固定常量场景与扰动先于结果提交于 `4a7c5b9`；运行偏离为 0。
- [MEASURED] `theta` 专用协议先于网格输出冻结于 `7a456ee`；结果发布于 `329b77e`。
- [MEASURED] 根目录 `/goal` 的 C-代理完成定义在 `5fdcd6c` 独立生效。
- [MEASURED] 根目录 `/goal` 的 C-结构完成定义在 `5fdcd6c` 独立生效；历史数值结果保持不变。
...
Markov/C/敏感性 78 项，共 118 项测试；三套环境均使用项目内 `.venv`。
三重天花板证据陈旧性与 manifest 哈希检查、`compileall`、`git diff --check`
同时通过。`scripts/check_c_semantics.py` 也已接入统一入口。
它检查 C-代理的目标、标题、概率语义和独立结论句。
它独立检查 C-结构的标题与逐时段裁决纪律。

## 项目当前边界

A 已形成 CMC/NGX 复现、DYC2 来源纠错和 UKMET 独立核心敏感性。
B 已把公开真值缺口量化为 0/108，并提供代理测量与风压工具。
C-代理已建立非退化强度波形 Brier 门槛。
C-结构已建立分级明确的巴威一手双环时段裁决。
